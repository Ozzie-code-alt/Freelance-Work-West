...
import { IndustrialTechcolumns } from '@/app/payments/IndustrialTechColumns';
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (!jsonResponse.hasOwnProperty('industrialTechs')) {
          throw new Error('Expected property industrialTech not found in the response');
        }

        const { industrialTechs } = jsonResponse;
        console.log('This is BAC Info Admin', industrialTechs);

        console.log('this is new data', industrialTechs);

        setData(industrialTechs);
      } catch (error) {
  ...
  return (
    <div className='container mx-auto h-auto  flex flex-col gap-5   py-10'>
      <div className='w-full  '>
        <GradualSpacing text='Industrial Technology Office Admin' className='text-center text-6xl' />
      </div>

      <div className=''>
        <DataTable columns={IndustrialTechcolumns} data={data} />
      </div>
