...
import { Guidancecolumns } from '@/app/payments/GuidanceColumn';
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (!jsonResponse.hasOwnProperty('guidances')) {
          throw new Error('Expected property guidance not found in the response');
        }

        const { guidances } = jsonResponse;
        console.log('This is BAC Info Admin', guidances);

        console.log('this is new data', guidances);
  ...
  return (
    <div className='container mx-auto h-auto  flex flex-col gap-5   py-10'>
      <div className='w-full  '>
        <GradualSpacing text='Guidance Office Admin' className='text-center text-6xl' />
      </div>

      <div className=''>
        <DataTable columns={Guidancecolumns} data={data} />
      </div>
    </div>
  );
}
