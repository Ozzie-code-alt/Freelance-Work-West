...
import { Awardscolumns } from '@/app/payments/AwardsColumns';
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (!jsonResponse.hasOwnProperty('awards')) {
          throw new Error('Expected property bac not found in the response');
        }

        const { awards } = jsonResponse;
        console.log('This is BAC Info Admin', awards);

        console.log('this is new data', awards);
  ...
  return (
    <div className='container mx-auto h-auto  flex flex-col gap-5   py-10'>
      <div className='w-full  '>
        <GradualSpacing text='Awards Office Admin' className='text-center text-6xl' />
      </div>

      <div className=''>
        <DataTable columns={Awardscolumns} data={data} />
      </div>
    </div>
  );
}
