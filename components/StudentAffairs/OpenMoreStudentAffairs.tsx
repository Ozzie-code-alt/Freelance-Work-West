...
export const StudentAffairsModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/studentAffairs/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("studentAffairsInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { studentAffairsInfo } = data;

        setAdminInfo(studentAffairsInfo);
      } catch (error) {
