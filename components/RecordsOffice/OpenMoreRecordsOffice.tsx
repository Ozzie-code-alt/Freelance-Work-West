...
export const RecordsOfficeOpenMore = ({ isOpen, setIsOpen, formIDValue }:CashierModalOpenMoreProps) => {
  const [medicalInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/recordsOffice/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("recordsInfo")) {
          throw new Error(
