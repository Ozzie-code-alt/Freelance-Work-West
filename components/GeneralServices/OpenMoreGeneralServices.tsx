...
export const GeneralServicesModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/generalServices/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("generalServicesInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { generalServicesInfo } = data;

        setAdminInfo(generalServicesInfo);
      } catch (error) {
