...
export const AffairsOfficeModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/affairsOffice/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("affarisOfficeInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { affarisOfficeInfo } = data;

        setAdminInfo(affarisOfficeInfo);
      } catch (error) {
