...
export const SOICTModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/soict/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("soictInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { soictInfo } = data;

        setAdminInfo(soictInfo);
      } catch (error) {
