...
export const GenderDevelopmentModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/genderDevelopment/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("genderDevelopmentInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { genderDevelopmentInfo } = data;

        setAdminInfo(genderDevelopmentInfo);
      } catch (error) {
