...
export const IndustrialTechModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/industrialTech/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("industrialTechInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { industrialTechInfo } = data;

        setAdminInfo(industrialTechInfo);
      } catch (error) {
