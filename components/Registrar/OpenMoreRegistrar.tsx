...
export const RegistrarModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/registrar/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("registrarInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { registrarInfo } = data;

        setAdminInfo(registrarInfo);
      } catch (error) {
