...
export const RMUModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/rmu/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("rmuInfo")) {
          throw new Error(
