...
export const PublicAffairsModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/publicAffairs/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("publicAffairsInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { publicAffairInfo } = data;

        setAdminInfo(publicAffairInfo);
      } catch (error) {
