...
export const CulturalModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/cultural/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("culturalInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { culturalInfo } = data;

        setAdminInfo(culturalInfo);
      } catch (error) {
