...
export const PhysicalPlantModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/physicalPlant/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("physicalPlantInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { physicalPlantInfo } = data;

        setAdminInfo(physicalPlantInfo);
      } catch (error) {
