...
export const BudgetModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/budget/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("budgetInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { budgetInfo } = data;

        setAdminInfo(budgetInfo);
      } catch (error) {
