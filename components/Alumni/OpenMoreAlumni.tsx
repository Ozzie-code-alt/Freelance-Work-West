...
export const AluimniModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/alumni/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("alumniInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { alumniInfo } = data;

        setAdminInfo(alumniInfo);
      } catch (error) {
