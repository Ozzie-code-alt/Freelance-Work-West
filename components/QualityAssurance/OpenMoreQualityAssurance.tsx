...
export const QualityAssuranceModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/qualityAssurance/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("qualityAssuranceInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { qualityAssuranceInfo } = data;

        setAdminInfo(qualityAssuranceInfo);
      } catch (error) {
