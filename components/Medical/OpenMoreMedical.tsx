...
export const MedicalModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:CashierModalOpenMoreProps) => {
  const [medicalInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/medical/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("medicalInfo")) {
          throw new Error(
