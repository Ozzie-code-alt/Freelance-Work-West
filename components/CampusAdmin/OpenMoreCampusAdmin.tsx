...
export const CampusAdminModalOpenMore = ({ isOpen, setIsOpen, formIDValue }:any) => {
  const [adminInfo, setAdminInfo] = useState({});

  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await fetch(`/api/campusAdmin/${formIDValue}`); // Make sure this URL is correct
        if (!response.ok) {
        ...
        if (!data.hasOwnProperty("campusAdminInfo")) {
          throw new Error(
            "Expected property adminInfo not found in the response"
          );
        }
        const { campusAdminInfo } = data;

        setAdminInfo(campusAdminInfo);
      } catch (error) {
